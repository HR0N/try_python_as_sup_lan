...
def fill_SOCAR():
    ...
    for item in socar:
        try:
            item.find('div', class_='lines_child').get_text(strip=True)
        except:
            send_telegram(
                "<b>[ where ]: </b> today-taxi\n"
                + "<b>[ section ]: </b> parsing fuel - Fill SOCAR\n"
            )
        title = item.find('div', class_='lines_child').get_text(strip=True)
        price = item.find('p', class_='two-image-and-content__price').get_text(strip=True).replace('*Цена: ', '').replace(' грн/л', '')
        if title == "F 100":
            array.append({"title": "KLO", "br": ""})
        if title == "Euro 95":
            array[0]["95+"] = price
...
# socar = parse_univ('https://socar.ua/ru/fuel', 'div', 'div', 'content-card-list', 'two-image-and-content')
# fill_SOCAR()
